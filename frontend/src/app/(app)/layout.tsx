...
type NavIconName = "review" | "today" | "soon" | "later" | "someday" | "chat" | "settings";
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const skipGates = pathname === "/triage" || pathname === "/winddown" || pathname === "/settings" || pathname === "/chat" || pathname === "/onboarding";
  ...
  const mainNavItems = navItems.filter((item) => item.icon !== "settings" && item.icon !== "chat");
  const chatItem = navItems.find((item) => item.icon === "chat")!;
      ...
      {!hideNav && (
        <nav className="fixed inset-y-0 left-0 w-56 bg-bg-card flex flex-col">
          ...
          <div className="mt-auto mx-3 pt-3 pb-4 border-t border-border/50 flex flex-col gap-0.5">
            ...
            <button
              onClick={() => router.push(chatItem.href)}
              aria-current={pathname === chatItem.href ? "page" : undefined}
              className={cn(
            ...
            >
              <NavIcon
                name={chatItem.icon}
                className={cn(pathname === chatItem.href && "text-accent-blue")}
              />
