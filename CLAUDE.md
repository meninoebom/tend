...
### Quadrant per-cell add: hover-revealed inline input, priority from the cell
Each Eisenhower cell has a `CellAddInput` (in `quadrant-layout.tsx`) at the foot of its body — a ghost `＋ add` row that's `opacity-0 group-hover:opacity-100` (invisible until the cell is hovered) on desktop, and forced visible + `min-h-[44px]` on touch via `[@media(hover:none)]:opacity-100`. Clicking it swaps to a bare `<input>` scoped to that quadrant: Enter runs the text through `parseCapture` then `createTask` with `important`/`urgent` **forced from the cell** (the cell wins over any stray `!`/`u!` token), `bucket` defaulting to `activeBucket`; it clears and keeps focus for rapid-fire, and Escape / empty-blur collapses back to the ghost row. This is deliberately lighter than the page-top `TaskInput` (no priority toggles — the cell already fixes priority) so four of them don't clutter the grid. The keep-it-clean rule: reveal per-cell affordances on hover, don't add always-on chrome. `QuadrantCell` needs `activeBucket` threaded in for this.
...
`lib/parse-capture.ts` is a **pure** function that pulls inline tokens out of a task string: `#domain` (prefix-matched against the user's domains, first match wins), `*` (important — star, like a starred email), `!` (urgent — bang = time pressure), `*!`/`!*` (important+urgent, the "do first" fast-path), `>today|soon|later|someday` (bucket), `~s|~m|~l` (size). Recognized tokens are stripped from the text; unrecognized `#tags` are left verbatim. Only a **standalone** token parses, so trailing punctuation ("Call mom!") stays text. The Tab-picker keys in `task-input.tsx` mirror the grammar (`*` important, `!` urgent) — keep them aligned if the token glyphs ever change. `task-input.tsx` submits on a single Enter (refocuses for rapid-fire; never disables the input) and opens the domain/priority picker on **Tab**, not Enter. The `/capture` PWA share-target page reuses the same parser.
