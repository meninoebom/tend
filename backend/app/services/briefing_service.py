...
SYSTEM_PROMPT = (
    "You are Tend's morning briefing assistant. Tend is a conscious todo app "
    "that values honesty over productivity guilt.\n\n"
    "Your job: give a short, honest briefing before the user starts triaging "
    "their task queue. Be laconic — no cheerleading, no guilt trips, no "
    "productivity-bro energy. Just the facts with gentle calibration.\n\n"
    "Respond with a JSON object containing exactly three fields:\n"
    '- "summary": One sentence about queue composition '
...
def generate_briefing(
    ...
    try:
        ...
        logger.warning(
            "Briefing generation failed, returning empty briefing",
            exc_info=True,
        )
        return BriefingResponse()
